...
import { X, Shield, User, Hash, Server, Phone, Trophy, ExternalLink } from 'lucide-react';
...
export default function TeamDetailModal({ team, onClose }: TeamDetailModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      {/* Modal */}
      <div className="relative z-10 w-full max-w-lg max-h-[90vh] overflow-y-auto glass-card rounded-2xl border border-neon-blue/20">
        {/* Header */}
        <div className="sticky top-0 flex items-center justify-between p-5 border-b border-dark-400/50 bg-dark-800/95 backdrop-blur-sm rounded-t-2xl z-10">
          <div className="flex items-center gap-3">
            {team.logo_url ? (
              <div className="w-10 h-10 rounded-lg overflow-hidden border border-neon-blue/20">
            ...
            <div>
              <h2 className="font-display font-bold text-white">{team.team_name}</h2>
            ...
            <div className="space-y-2">
              {corePlayers.map((player, idx) => (
                <div key={player.id} className={cn(
                  ...
                  idx === 0 ? 'bg-neon-gold/5 border border-neon-gold/15' : 'bg-dark-600/50'
                )}>
                  <div className={cn(
                    'w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold shrink-0',
                    idx === 0 ? 'bg-neon-gold/20 text-neon-gold border border-neon-gold/30' : 'bg-neon-blue/10 text-neon-blue border border-neon-blue/20'
                  )}>
            ...
            <div>
              ...
              <div className="space-y-2">
                {subPlayers.map((player) => (
                  <div key={player.id} className="flex items-center gap-3 p-3 rounded-lg bg-dark-600/50 border border-neon-purple/10">
                    <div className="w-7 h-7 rounded-full bg-neon-purple/10 border border-neon-purple/20 flex items-center justify-center text-xs font-bold text-neon-purple shrink-0">
                      C
                    </div>
                    <div className="flex-1 min-w-0">
