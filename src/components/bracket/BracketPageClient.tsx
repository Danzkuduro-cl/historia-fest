...
import { 
  Trophy, ChevronLeft, RotateCcw, Shuffle, Sparkles, 
...
export default function BracketPageClient({ initialTeams }: BracketPageClientProps) {
  ...
  const handleAutoFill = (shuffle: boolean = false) => {
    ...
    let teamList = initialTeams.map(t => t.team_name);
    if (shuffle) {
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-900 flex flex-col print:bg-white print:p-0">
      ...
      <header className="sticky top-0 z-40 bg-white border-b border-slate-200 shadow-sm print:hidden">
        <div className="max-w-7xl mx-auto px-4 h-16 flex items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            <Link
              href="/"
              className="flex items-center gap-1.5 text-slate-600 hover:text-red-600 font-semibold text-xs transition-colors py-1.5 px-2.5 rounded-lg hover:bg-slate-100"
            >
            ...
            <div className="flex items-center gap-2">
              ...
              <div>
                <h1 className="font-display font-bold text-sm md:text-base leading-none text-slate-900">
                  BAGAN TURNAMEN FIESTA HISTORIA
                </h1>
                <p className="text-[10px] text-red-600 font-mono font-bold mt-0.5">
                  59 TIM · SINGLE ELIMINATION · 5 BYE SLOTS
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className="hidden sm:flex items-center gap-1.5 px-3 py-1.5 bg-slate-100 hover:bg-slate-200 text-slate-700 text-xs font-bold rounded-lg border border-slate-300 transition"
            >
              <Sparkles className="w-3.5 h-3.5 text-red-600" />
              <span>Isi dari DB</span>
            </button>

            <button
              onClick={() => handleAutoFill(true)}
              className="hidden sm:flex items-center gap-1.5 px-3 py-1.5 bg-red-50 hover:bg-red-100 text-red-600 text-xs font-bold rounded-lg border border-red-200 transition"
            >
              <Shuffle className="w-3.5 h-3.5" />
              <span>Acak Undian</span>
            </button>

            <button
              onClick={() => window.print()}
              className="p-2 sm:px-3 sm:py-1.5 bg-white hover:bg-slate-50 text-slate-700 text-xs font-bold rounded-lg border border-slate-200 shadow-sm transition flex items-center gap-1.5"
            >
            ...
            <button
              ...
              className="p-2 sm:px-3 sm:py-1.5 bg-slate-100 hover:bg-red-50 text-slate-500 hover:text-red-600 text-xs font-bold rounded-lg border border-slate-200 transition flex items-center gap-1.5"
            >
      ...
      {/* Tabs */}
      <section className="bg-white border-b border-slate-200 py-3 px-4 print:hidden sticky top-16 z-30 shadow-xs">
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row items-center justify-between gap-3">
          <div className="flex items-center gap-2 overflow-x-auto max-w-full pb-1 md:pb-0">
            <button
              onClick={() => setViewMode('poolA')}
              className={cn(
                "px-4 py-2 rounded-xl text-xs font-display font-bold transition flex items-center gap-2 shrink-0 border",
                viewMode === 'poolA'
            ...
            >
              ...
              <span>Sesi 1: Pool A (Sesi Pagi)</span>
            </button>
            ...
            >
              ...
              <span>Sesi 2: Pool B (Sesi Siang)</span>
            </button>
          ...
          <div className="flex items-center gap-3 text-xs font-mono text-slate-600">
            <span className="bg-red-50 text-red-700 px-2.5 py-1 rounded-lg border border-red-200 font-bold">
              Match Selesai: {stats.completedMatches} / {stats.totalMatches}
            </span>
          </div>
        </div>
      </section>
      ...
      {/* Main Container */}
      <main className="flex-1 p-4 md:p-6 overflow-x-auto max-w-7xl mx-auto w-full">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 items-start">
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-6 pt-4">
                  {Array.from({ length: 2 }, (_, i) => {
                    const sfNum = i + 1;
                    return (
                      <div key={`finals-sf-${sfNum}`} className="bg-white border-2 border-red-200 rounded-xl p-3 shadow-xs space-y-2">
              ...
              <div className="space-y-4">
                ...
                <div className="bg-slate-900 text-white p-4 rounded-2xl border border-red-600/30 text-center space-y-2">
                  <h4 className="font-display font-bold text-xs uppercase text-amber-400">🏆 PODIUM JUARA</h4>
                ...
                <table className="w-full text-left text-xs">
                  ...
                  <tbody className="divide-y divide-slate-100 font-body">
                    {filteredMatches.map((m) => {
                      ...
                      return (
                        <tr key={`list-${m.id}`} className="hover:bg-slate-50/80 transition-colors">
      ...
      {/* Footer */}
      <footer className="bg-white border-t border-slate-200 py-4 px-6 text-center text-xs text-slate-400 font-body print:hidden">
        <p>Bagan Turnamen Resmi Mobile Legends Piala Bupati Fiesta Historia 2026 • Data otomatis tersimpan di browser</p>
      </footer>
    </div>
  );
}
